...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* 當卡片需要獨立一列時的額外調整 */
        .wish-row.single-card { justify-content: center; width: 100%; }
        .wish-row.single-card .wish-card { width: 680px; max-width: 100%; }
        ...
        .wish-card-hero-image-wrapper {
            ...
            height: 180px; overflow: hidden; border-bottom: 1px solid rgba(255, 255, 255, 0.1);
        ...
        @media (max-width: 768px) {
            ...
            .wish-row.single-card .wish-card { width: 100%; }
            @keyframes scrollUp { 
                0% { transform: translateY(150px); } 
        ...
        .marquee-input::placeholder {
            ...
            overflow: hidden;
            width: 100%;
            animation: placeholderMarquee 12s linear infinite;
        }
...
<body>
    ...
    <script>
        ...
        function parseContentFilesToTable(contentHtml) {
            ...
            // 🚀 檢查是否含有原生多媒體標籤
            const hasNativeImg = tempDiv.querySelectorAll('img').length > 0;
            const hasNativeVideo = tempDiv.querySelectorAll('video').length > 0;
            ...
            // 🚀 綜合判定此張卡片是否內含有圖片或影音（包含 Table、首圖或原生多媒體標籤）
            const containsMedia = (filesArray.length > 0) || !!heroImageUrl || hasNativeImg || hasNativeVideo;
        ...
        function renderMarquee() {
            ...
            wishesData.forEach(wish => {
                ...
                if (parsed.containsMedia && !isMobile) {
                    // 🚀 如果目前排隊的列裡有卡片，先將它們推到大軌道
                    if (currentRow.length > 0) {
                        pushRowToTrack(currentRow, false);
                        currentRow = [];
                    }
                    // 將這張含有媒體的卡片獨立成一個單獨一列
                    pushRowToTrack([wish], true);
                } else {
                    // 一般卡片，依據指定格數排滿再換列
                    currentRow.push(wish);
            ...
            if (currentRow.length > 0) {
                if (!isMobile) {
                    // 補齊剩餘名額以利畫面對齊
                    const needAdds = itemsPerRow - currentRow.length;
                    for (let i = 0; i < needAdds; i++) {
                        currentRow.push(wishesData[wishesData.length - 1 - i] || wishesData[0]);
                    }
                }
                pushRowToTrack(currentRow, false);
            }

            // 輔助函式：建立與渲染 Wish Row Dom 節點
            function pushRowToTrack(items, isSingleCardRow) {
                const rowDiv = document.createElement('div');
                rowDiv.className = isSingleCardRow ? 'wish-row single-card' : 'wish-row';
                items.forEach(item => rowDiv.appendChild(createCardNode(item)));
                marqueeTrack.appendChild(rowDiv);
            }

            const totalRows = marqueeTrack.children.length;
