...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 確實執行新插入一筆資料 (INSERT)，指定新 ID 並且寫入 login_time 欄位
    $stmt = $conn->prepare("INSERT INTO makeawish (ID, name, number_of_houses, emperor_shizu, generation, message_of_blessing, login_time) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isiiiss", $insert_id, $name, $number_of_houses, $emperor_shizu, $generation_val, $message_of_blessing, $login_time);
...
<body>
    ...
    <script>
        ...
        // ==========================================
        // AJAX 監聽：依照「您的姓名」去 members 資料表撈取房數，自動塞進隱藏欄位
        // ==========================================
        document.getElementById('author').addEventListener('change', function() {
            const memberName = this.value.trim();
            if (memberName !== '') {
                fetch(`?action=get_houses&new_member=${encodeURIComponent(memberName)}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('hidden_houses').value = data.number_of_houses;
                        console.log(`AJAX 房數載入成功: ${data.number_of_houses}`);
                    })
                    .catch(error => {
        ...
        // ==========================================
        // 🛠️ 監聽表單送出事件：在「按鈕送出去前」捕捉系統現在時間並格式化寫入隱藏欄位
        // ==========================================
        document.getElementById('wishForm').addEventListener('submit', function() {
            ...
            // 拼接成符合規格要求的 'YYYY-MM-DD HH:MM:SS'
            const formattedTime = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`;
            
            // 把值塞進隱藏欄位，隨後表單便會帶著這個時間一同 POST 出去
